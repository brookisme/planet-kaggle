...
# inspect
pprint(kgres.graph)
kgres.load_weights('tdn-brvw-real-5.hdf5')
kgres.model().summary()
...
PKR=os.environ['PKR']
train_csv_path=f'{PKR}/datacsvs/aug_train.1.csv'
valid_csv_path=f'{PKR}/datacsvs/aug_valid.1.csv'
train_gen=DFGen(csv_file=train_csv_path,csv_sep=',',batch_size=train_batch_size,lambda_func=norm_brvw)
valid_gen=DFGen(csv_file=valid_csv_path,csv_sep=',',batch_size=valid_batch_size,lambda_func=norm_brvw)

train_gen.limit(int(train_gen.size*0.2))
valid_gen.limit(int(valid_gen.size*0.2))

aug_train_csv_path='train-aug_1_20pct.csv'
aug_valid_csv_path='valid-aug_1_20pct.csv'

train_gen.save(path=aug_train_csv_path)
valid_gen.save(path=aug_valid_csv_path)
